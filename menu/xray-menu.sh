...
function delete-user() {
	...
	uuid="$(cat /etc/mon/xray/clients.txt | grep -w "$user" | awk '{print $2}')"
    ...
    rm /etc/mon/config-user/${user}
    rm /etc/mon/config-url/${uuid}
	systemctl restart xray.service
...
function user-list() {
	...
	do
		...
		printf "%-17s %2s\n" "$user" "     $exp_date"
	done < /etc/mon/xray/clients.txt
	total=$(wc -l /etc/mon/xray/clients.txt | awk '{print $1}')
    echo -e "\e[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
...
function user-monitor() {
	...
	data=($(cat /etc/mon/xray/clients.txt | awk '{print $1}'));
	echo -e "\e[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
...
function show-config() {
    ...
    xtls="$(cat ~/log-install.txt | grep -w "XRAY VLESS XTLS SPLICE" | cut -d: -f2|sed 's/ //g')"
    link=$(cat /etc/mon/config-user/${user})
	uuid=$(cat /etc/mon/xray/clients.txt | grep -w "$user" | awk '{print $2}')
	domain=$(cat /etc/mon/xray/domain)
	exp=$(cat /etc/mon/xray/clients.txt | grep -w "$user" | awk '{print $3}')
	exp_date=$(date -d"${exp}" "+%d %b %Y")
...
function change-port() {
    ...
    if [[ -z $cek ]]; then
    sed -i "s/$xtls/$xtls1/g" /etc/mon/xray/conf/02_VLESS_TCP_inbounds.json
...
echo -e "\033[30;5;47m                  ⇱ XRAY CORE MENU ⇲              \033[m"
echo -e "\033[5;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\e[37m"
echo -e ""   
echo -e " [${CYAN}•1${NC}] Add XRAY user "
echo -e " [${CYAN}•2${NC}] Delete XRAY user "
echo -e " [${CYAN}•3${NC}] Extend XRAY user "
echo -e " [${CYAN}•4${NC}] View user list "
echo -e " [${CYAN}•5${NC}] Monitor user "
echo -e " [${CYAN}•6${NC}] Show User config "
echo -e " [${CYAN}•7${NC}] Change Port XRAY "
echo -e ""
echo -e " [${RED}•0${NC}] ${RED}MENU UTAMA${NC}"
echo -e   ""
